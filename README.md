...
The emulator has the Level Input and Level Output wired up to memory address `2^64 - 1`, which is why the assembler programs refer to `li fp, -1; lbu rd, 0(fp)` and the C programs refer to `IO = (volatile uint8_t*)(intptr_t)-1; x = *IO;`.
