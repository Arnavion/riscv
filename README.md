...
The emulator has the Level Input and Level Output wired up to memory address `2^32 - 1`, which is why the assembler programs refer to `lbu rd, -1(zero)` and the C programs refer to `IO = (volatile uint8_t*)(intptr_t)-1; x = *IO;`.
