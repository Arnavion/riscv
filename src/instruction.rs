...
macro_rules! instructions {
	(
		...
		impl $ty {
			...
			$vis fn parse($parse_line: &str, $supported_extensions: SupportedExtensions) -> Result<Option<Self>, ParseError<'_>> {
	...
	pub fn encode(self, supported_extensions: SupportedExtensions) -> Result<(u16, Option<u16>), EncodeError> {
		...
		let raw_instruction = match self {
			...
			Self::Lui { dest, imm } if
				...
				imm != 0 &&
				can_truncate_low::<12>(imm) &&
				can_truncate_high::<18>(imm)
			=> RawInstruction::Ci {
				opcode: OpCodeC::Lui,
				rd_rs1: dest,
				imm1: bit_slice::<12, 17>(imm),
				imm2: bit_slice::<17, 18>(imm),
			},
	...
	fn encode(self) -> Result<(u16, Option<u16>), EncodeError> {
		...
		let encoded = match self {
			...
			Self::U { opcode, rd, imm } => {
				// gas expects both imm = 0xfffff (20 bits, no sign extension consideration) and
				// imm = 0xffffe000 (32 bits with upper 20 bits populated) to work.
				let imm =
					if (imm as u32 & ((1 << 20) - 1)) == imm as u32 {
						bit_slice::<0, 20>(imm) << 12
					}
					else if can_truncate_low::<12>(imm) {
						bit_slice::<12, 32>(imm) << 12
					}
					else if can_truncate_high::<20>(imm) {
						bit_slice::<0, 20>(imm) << 12
					}
					else {
						return Err(EncodeError::ImmediateOverflow);
