fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	for instruction in riscv::parse_program(&program, supported_extensions) {
		...
		if let Some(hi) = hi {
			if save_breaker {
				println!("<U32>0x{hi:04x}{lo:04x} // {instruction}");
			}
			else {
				println!("0x{lo:04x} 0x{hi:04x} # {instruction}");
			}
		}
		else {
			if save_breaker {
				println!("<U16>0x{lo:04x}     // {instruction}");
			}
			else {
				println!("0x{lo:04x}        # {instruction}");
			}
		}
	}
...
fn parse_args(mut args: impl Iterator<Item = std::ffi::OsString>, argv0: &std::ffi::OsStr) -> (std::path::PathBuf, riscv::SupportedExtensions, bool) {
	...
	for opt in &mut args {
		match opt.to_str() {
			...
			Some("-c" | "--compress" | "--compress=true") => supported_extensions |= riscv::SupportedExtensions::RVC,

			Some("--compress=false") => supported_extensions &= !riscv::SupportedExtensions::ZCB,

			Some("--compress=Zcb") => supported_extensions |= riscv::SupportedExtensions::ZCB,
...
fn write_usage(mut w: impl std::io::Write, argv0: &std::ffi::OsStr) {
	_ = writeln!(w, "Usage: {} [ --32 | --64 ] [ -c | --compress | --compress=[true|false|Zcb] ] [ --sb | --save-breaker ] [ --zba ] [ -- ] <program.S>", argv0.to_string_lossy());
}
