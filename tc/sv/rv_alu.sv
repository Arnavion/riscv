...
module rv_alu (
	...
	cmp cmp_module (
		...
		.lt(cmp_lt), .eq(cmp_eq), .ne(cmp_ne), .ge(cmp_ge)
	);
...
module logical (
	...
	assign out_and = arg1 & ({64{invert_arg2}} ^ arg2);
	assign out_or = arg1 | ({64{invert_arg2}} ^ arg2);
	assign out_xor = arg1 ^ ({64{invert_arg2}} ^ arg2);
endmodule
