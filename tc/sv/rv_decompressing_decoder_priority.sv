module rv_decompressing_decoder_priority #(
	...
	`define imm {imm_31_20, imm_19_18, imm_17, imm_16_12, imm_11, imm_10, imm_9, imm_8, imm_7, imm_6, imm_5, imm_4, imm_3, imm_2, imm_1, imm_0}
	...
	always_comb begin
		...
		priority casez (in[0+:16])
			...
			16'b000_????????_???_00: begin
				...
				`imm = 32'({in[7+:4], in[11+:2], in[5], in[6], 2'b0});
			end
			...
			16'b000_?_?????_?????_01: begin
				...
				`imm = unsigned'(32'(signed'({in[12], in[2+:5]})));
			end
			...
			16'b001_?_?????_?????_01: if (rv64) begin
				...
				`imm = unsigned'(32'(signed'({in[12], in[8], in[9+:2], in[6], in[7], in[2], in[11], in[3+:3], 1'b0})));
			end
			...
			16'b011_?_00010_?????_01: begin
				...
				`imm = unsigned'(32'(signed'({in[12], in[3+:2], in[5], in[2], in[6], 4'b0})));
			end
			...
			16'b011_?_?????_?????_01: begin
				...
				`imm = unsigned'(32'(signed'({in[12], in[2+:5], 12'b0})));
			end

			// srli
			16'b100_0_00_???_?????_01: begin
				opcode = OpCode_OpImm;
				funct3 = 3'b101;
			...
			// srli
			16'b100_1_00_???_?????_01: if (rv64) begin
				opcode = OpCode_OpImm;
				funct3 = 3'b101;
				funct7 = {6'b000000, in[12]};
			...
			16'b100_0_01_???_?????_01: begin
				...
				`imm = 32'({5'b10000, in[12], in[2+:5]});
			end

			// srai
			16'b100_1_01_???_?????_01: if (rv64) begin
				opcode = OpCode_OpImm;
				funct3 = 3'b101;
				funct7 = {6'b010000, in[12]};
			...
			16'b100_1_11_???_00_???_01: if (rv64) begin
				// subw
				opcode = OpCode_Op32;
				funct3 = 3'b000;
				funct7 = 7'b0100000;
			...
			16'b100_1_11_???_01_???_01: if (rv64) begin
				// addw
				opcode = OpCode_Op32;
				funct3 = 3'b000;
				funct7 = 7'b000000;
			...
			// sext.b
			16'b100_1_11_???_11_001_01: begin
				opcode = OpCode_OpImm;
				funct3 = 3'b001;
				funct7 = 7'b0110000;
			...
			// sext.h
			16'b100_1_11_???_11_011_01: begin
				opcode = OpCode_OpImm;
				funct3 = 3'b001;
				funct7 = 7'b0110000;
			...
			16'b110_???_???_?????_01: begin
				...
				`imm = unsigned'(32'(signed'({in[12], in[5+:2], in[2], in[10+:2], in[3+:2], 1'b0})));
			end
			...
			16'b010_?_?????_?????_10: begin
				...
				`imm = 32'({in[2+:2], in[12], in[4+:3], 2'b0});
			end
			...
			16'b011_?_?????_?????_10: if (rv64) begin
				...
				`imm = 32'({in[2+:3], in[12], in[5+:2], 3'b0});
			end else begin
				// flwsp
				sigill = '1;
				is_compressed = 'x;
			...
			16'b?_???_?????_11001_11: begin
				...
				`imm = unsigned'(32'(signed'(in[20+:12])));
			end
			...
			16'b?_???_?????_01000_11: begin
				...
				`imm = unsigned'(32'(signed'({in[25+:7], in[7+:5]})));
			end
			...
			16'b?_???_?????_11000_11: begin
				...
				`imm = unsigned'(32'(signed'({in[31], in[7], in[25+:6], in[8+:4], 1'b0})));
			end
			...
			16'b?_???_?????_11011_11: begin
				...
				`imm = unsigned'(32'(signed'({in[31], in[12+:8], in[20], in[21+:10], 1'b0})));
			end
