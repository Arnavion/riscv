module load_store (
	...
	bit[3:0] store_mask;
	bit[31:0] store_value_shifted;

	wire[3:0] address2_decoded = 4'b1 << address;
	...
	always_comb begin
		...
		if (efault) begin
			...
			unique case (funct3[0+:2])
				2'b00: load_value[8+:24] = {24{~funct3[2] & load_value[7]}};
				2'b01: load_value[16+:16] = {16{~funct3[2] & load_value[15]}};
				2'b10,
				2'b11: ;
			endcase
			...
			if (funct3[2]) begin
				...
				store_mask = {
					...
					funct3[1] | (funct3[0] & address2_decoded[2]) | address2_decoded[3],
					// {funct3[0+:2], address} inside {4'b10_??, 4'b01_1?, 4'b00_10},
					funct3[1] | (funct3[0] & address2_decoded[3]) | address2_decoded[2],
					// {funct3[0+:2], address} inside {4'b10_??, 4'b01_0?, 4'b00_01},
					funct3[1] | (funct3[0] & address2_decoded[0]) | address2_decoded[1],
					// {funct3[0+:2], address} inside {4'b10_??, 4'b01_0?, 4'b00_00}
					funct3[1] | (funct3[0] & address2_decoded[1]) | address2_decoded[0]
				};

				unique case (funct3[0+:2])
					2'b00: store_value_shifted = {4{store_value[0+:8]}};
					2'b01: store_value_shifted = {2{store_value[0+:16]}};
