...
+---------+------+-------+-------+-----------------+-------+----+-------+-----------------+------+-----------------+-------+-----------------+------+-------+-----------------+-----------------+
| Xfer    |      |       |       |                 | 1     | -> | Xfer  | Current Address | 1    |                 | 0     |                 | 0    | 0     |                 |                 |
| Xfer    |      |       |       |                 | 0     | -> | Clean | Current Address | 1    |                 | 1     | Slow Load Value | 0    | 0     |                 |                 |
+---------+------+-------+-------+-----------------+-------+----+-------+-----------------+------+-----------------+-------+-----------------+------+-------+-----------------+-----------------+
		...
		unique case (state)
			State_Clean: unique case (ram_input)
				...
				RamInput_Load: if (ram_block_address == cached_ram_block_address) begin
					load_value = fast_load_value[{isa_block_address, {$clog2(xlen){1'b0}}}+:xlen];
				end else begin
				...
				RamInput_Store: if (ram_block_address == cached_ram_block_address) begin
					...
					fast_store_value[{isa_block_address, {$clog2(xlen){1'b0}}}+:xlen] = store_value;
				end else begin
					next_state = State_Xfer;
					next_cached_ram_block_address = ram_block_address;
					busy = '1;
...
module test_ram_cache #(
	...
	ram_cache #(.rv64(rv64), .ram_block_width(ram_block_width)) ram_cache_module (
		clock, reset,
		address, ram_input, store_value,
		fast_load_value,
		slow_load_ready, slow_load_value,
		...
		slow_load, slow_load_address, slow_store, slow_store_address
	);
