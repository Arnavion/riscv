module rv_decompressor_priority #(
	...
	always_comb begin
		...
		priority casez (in[0+:16])
			...
			// fld
			16'b001_???_???_??_???_00: type_i(OpCode_LoadFp, {2'b01, in[2+:3]}, 3'b011, {2'b01, in[7+:3]}, 12'({in[5+:2], in[10+:3], 3'b000}));

			// lw
			16'b010_???_???_??_???_00: type_i(OpCode_Load, {2'b01, in[2+:3]}, 3'b010, {2'b01, in[7+:3]}, 12'({in[5], in[10+:3], in[6], 2'b00}));
			...
			// lbu
			16'b100_000_???_??_???_00: type_i(OpCode_Load, {2'b01, in[2+:3]}, 3'b100, {2'b01, in[7+:3]}, 12'({in[5], in[6]}));

			// lhu / lh
			16'b100_001_???_??_???_00: type_i(OpCode_Load, {2'b01, in[2+:3]}, {!in[6], 2'b01}, {2'b01, in[7+:3]}, 12'({in[5], 1'b0}));

			// sb
			16'b100_010_???_??_???_00: type_s(OpCode_Store, 3'b000, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 12'({in[5], in[6]}));

			// sh
			16'b100_011_???_??_???_00: type_s(OpCode_Store, 3'b001, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 12'({in[5], 1'b0}));
			...
			// fsd
			16'b101_???_???_??_???_00: type_s(OpCode_StoreFp, 3'b011, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 12'({in[5+:2], in[10+:3], 3'b000}));

			// sw
			16'b110_???_???_??_???_00: type_s(OpCode_Store, 3'b010, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 12'({in[5], in[10+:3], in[6], 2'b00}));
			...
			// addi
			16'b000_?_?????_?????_01: type_i(OpCode_OpImm, in[7+:5], 3'b000, in[7+:5], unsigned'(12'(signed'({in[12], in[2+:5]}))));
			...
			// li
			16'b010_?_?????_?????_01: type_i(OpCode_OpImm, in[7+:5], 3'b000, 5'b00000, unsigned'(12'(signed'({in[12], in[2+:5]}))));
			...
			// beqz
			16'b110_???_???_?????_01: type_b(OpCode_Branch, 3'b000, {2'b01, in[7+:3]}, 5'b00000, unsigned'(12'(signed'({in[12], in[5+:2], in[2], in[10+:2], in[3+:2]}))));

			// bnez
			16'b111_???_???_?????_01: type_b(OpCode_Branch, 3'b001, {2'b01, in[7+:3]}, 5'b00000, unsigned'(12'(signed'({in[12], in[5+:2], in[2], in[10+:2], in[3+:2]}))));

			// slli
			16'b000_?_?????_?????_10: type_i(OpCode_OpImm, in[7+:5], 3'b001, in[7+:5], {6'b000000, in[12], in[2+:5]});

			// fldsp
			16'b001_?_?????_?????_10: type_i(OpCode_LoadFp, in[7+:5], 3'b011, 5'b00010, 12'({in[2+:3], in[12], in[5+:2], 3'b000}));

			// lwsp
			16'b010_?_?????_?????_10: type_i(OpCode_Load, in[7+:5], 3'b010, 5'b00010, 12'({in[2+:2], in[12], in[4+:3], 2'b00}));
			...
			// fsdsp
			16'b101_??????_?????_10: type_s(OpCode_StoreFp, 3'b011, 5'b00010, in[2+:5], 12'({in[7+:3], in[10+:3], 3'b000}));

			// swsp
			16'b110_??????_?????_10: type_s(OpCode_Store, 3'b010, 5'b00010, in[2+:5], 12'({in[7+:2], in[9+:4], 2'b00}));
