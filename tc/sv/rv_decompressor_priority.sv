module rv_decompressor_priority #(
	...
	assign out = {out_31, out_30_25, out_24_23, out_22_21, out_20, out_19_18, out_17_15, out_14_12, out_11_10, out_9_8, out_7, out_6_2, 2'b11};

	function automatic void type_r(bit[4:0] opcode, bit[4:0] rd, bit[2:0] funct3, bit[4:0] rs1, bit[4:0] rs2, bit[6:0] funct7);
		out_6_2 = opcode;
		{out_11_10, out_9_8, out_7} = rd;
		out_14_12 = funct3;
		{out_19_18, out_17_15} = rs1;
		{out_24_23, out_22_21, out_20} = rs2;
		{out_31, out_30_25} = funct7;
	endfunction
	...
	function automatic void type_i(bit[4:0] opcode, bit[4:0] rd, bit[2:0] funct3, bit[4:0] rs1, bit[11:0] imm);
		...
		out_14_12 = funct3;
		{out_19_18, out_17_15} = rs1;
		{out_31, out_30_25, out_24_23, out_22_21, out_20} = imm;
	endfunction
	...
	function automatic void type_s(bit[4:0] opcode, bit[2:0] funct3, bit[4:0] rs1, bit[4:0] rs2, bit[11:0] imm);
		...
		{out_19_18, out_17_15} = rs1;
		{out_24_23, out_22_21, out_20} = rs2;
		{out_31, out_30_25, out_11_10, out_9_8, out_7} = imm;
	endfunction
	...
	function automatic void type_b(bit[4:0] opcode, bit[2:0] funct3, bit[4:0] rs1, bit[4:0] rs2, bit[11:0] imm);
		...
		{out_19_18, out_17_15} = rs1;
		{out_24_23, out_22_21, out_20} = rs2;
		{out_31, out_7, out_30_25, out_11_10, out_9_8} = imm;
	endfunction

	function automatic void type_u(bit[4:0] opcode, bit[4:0] rd, bit[19:0] imm);
		out_6_2 = opcode;
		{out_11_10, out_9_8, out_7} = rd;
		{out_31, out_30_25, out_24_23, out_22_21, out_20, out_19_18, out_17_15, out_14_12} = imm;
	endfunction

	function automatic void type_j(bit[4:0] opcode, bit[4:0] rd, bit[19:0] imm);
		out_6_2 = opcode;
		{out_11_10, out_9_8, out_7} = rd;
		{out_31, out_19_18, out_17_15, out_14_12, out_20, out_30_25, out_24_23, out_22_21} = imm;
	endfunction

	always_comb begin
		sigill = '0;
		is_compressed = ~& in[0+:2];
		{out_31, out_30_25, out_24_23, out_22_21, out_20, out_19_18, out_17_15, out_14_12, out_11_10, out_9_8, out_7, out_6_2} = 'x;
		...
		priority casez (in[0+:16])
			...
			// addi4spn
			16'b000_????????_???_00: type_i(OpCode_OpImm, {2'b01, in[2+:3]}, 3'b000, 5'b00010, {2'b00, in[7+:4], in[11+:2], in[5], in[6], 2'b00});

			// fld
			16'b001_???_???_??_???_00: type_i(OpCode_LoadFp, {2'b01, in[2+:3]}, 3'b011, {2'b01, in[7+:3]}, {4'b0000, in[5+:2], in[10+:3], 3'b000});

			// lw
			16'b010_???_???_??_???_00: type_i(OpCode_Load, {2'b01, in[2+:3]}, 3'b010, {2'b01, in[7+:3]}, {5'b00000, in[5], in[10+:3], in[6], 2'b00});

			16'b011_???_???_??_???_00: if (rv64)
				// ld
				type_i(OpCode_Load, {2'b01, in[2+:3]}, 3'b011, {2'b01, in[7+:3]}, {4'b0000, in[5+:2], in[10+:3], 3'b000});
			else
				// flw
				type_i(OpCode_LoadFp, {2'b01, in[2+:3]}, 3'b010, {2'b01, in[7+:3]}, {5'b00000, in[5], in[10+:3], in[6], 2'b00});
			...
			// fsd
			16'b101_???_???_??_???_00: type_s(OpCode_StoreFp, 3'b011, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, {4'b0000, in[5+:2], in[10+:3], 3'b000});

			// sw
			16'b110_???_???_??_???_00: type_s(OpCode_Store, 3'b010, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, {5'b00000, in[5], in[10+:3], in[6], 2'b00});

			16'b111_???_???_??_???_00: if (rv64)
				// sd
				type_s(OpCode_Store, 3'b011, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, {4'b0000, in[5+:2], in[10+:3], 3'b000});
			else
				// fsw
				type_s(OpCode_StoreFp, 3'b010, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, {5'b00000, in[5], in[10+:3], in[6], 2'b00});

			// addi
			16'b000_?_?????_?????_01: type_i(OpCode_OpImm, in[7+:5], 3'b000, in[7+:5], {{7{in[12]}}, in[2+:5]});

			16'b001_?_?????_?????_01: if (rv64)
				// addiw
				type_i(OpCode_OpImm32, in[7+:5], 3'b000, in[7+:5], {{7{in[12]}}, in[2+:5]});
			else
				// jal
				type_j(OpCode_Jal, 5'b00001, {{10{in[12]}}, in[8], in[9+:2], in[6], in[7], in[2], in[11], in[3+:3]});

			// li
			16'b010_?_?????_?????_01: type_i(OpCode_OpImm, in[7+:5], 3'b000, 5'b00000, {{7{in[12]}}, in[2+:5]});

			// addi16sp
			16'b011_?_00010_?????_01: type_i(OpCode_OpImm, 5'b00010, 3'b000, 5'b00010, {{3{in[12]}}, in[3+:2], in[5], in[2], in[6], 4'b0000});

			// lui
			16'b011_?_?????_?????_01: type_u(OpCode_Lui, in[7+:5], {{15{in[12]}}, in[2+:5]});

			// srli
			16'b100_?_00_???_?????_01: type_i(OpCode_OpImm, {2'b01, in[7+:3]}, 3'b101, {2'b01, in[7+:3]}, {6'b000000, in[12], in[2+:5]});

			// srai
			16'b100_?_01_???_?????_01: type_i(OpCode_OpImm, {2'b01, in[7+:3]}, 3'b101, {2'b01, in[7+:3]}, {6'b010000, in[12], in[2+:5]});

			// andi
			16'b100_?_10_???_?????_01: type_i(OpCode_OpImm, {2'b01, in[7+:3]}, 3'b111, {2'b01, in[7+:3]}, {{7{in[12]}}, in[2+:5]});

			// sub
			16'b100_0_11_???_00_???_01: type_r(OpCode_Op, {2'b01, in[7+:3]}, 3'b000, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 7'b0100000);

			// xor
			16'b100_0_11_???_01_???_01: type_r(OpCode_Op, {2'b01, in[7+:3]}, 3'b100, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 7'b0000000);

			// or
			16'b100_0_11_???_10_???_01: type_r(OpCode_Op, {2'b01, in[7+:3]}, 3'b110, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 7'b0000000);

			// and
			16'b100_0_11_???_11_???_01: type_r(OpCode_Op, {2'b01, in[7+:3]}, 3'b111, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 7'b0000000);

			16'b100_1_11_???_00_???_01: if (rv64)
				// subw
				type_r(OpCode_Op32, {2'b01, in[7+:3]}, 3'b000, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 7'b0100000);
			else begin
				sigill = '1;
				is_compressed = 'x;
			end

			16'b100_1_11_???_01_???_01: if (rv64)
				// addw
				type_r(OpCode_Op32, {2'b01, in[7+:3]}, 3'b000, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 7'b0000000);
			else begin
				sigill = '1;
				is_compressed = 'x;
			end
			...
			// j
			16'b101_???????????_01: type_j(OpCode_Jal, 5'b00000, {{10{in[12]}}, in[8], in[9+:2], in[6], in[7], in[2], in[11], in[3+:3]});

			// beqz
			16'b110_???_???_?????_01: type_b(OpCode_Branch, 3'b000, {2'b01, in[7+:3]}, 5'b00000, {{5{in[12]}}, in[5+:2], in[2], in[10+:2], in[3+:2]});

			// bnez
			16'b111_???_???_?????_01: type_b(OpCode_Branch, 3'b001, {2'b01, in[7+:3]}, 5'b00000, {{5{in[12]}}, in[5+:2], in[2], in[10+:2], in[3+:2]});

			// slli
			16'b000_?_?????_?????_10: type_i(OpCode_OpImm, in[7+:5], 3'b001, in[7+:5], {6'b000000, in[12], in[2+:5]});

			// fldsp
			16'b001_?_?????_?????_10: type_i(OpCode_LoadFp, in[7+:5], 3'b011, 5'b00010, {3'b000, in[2+:3], in[12], in[5+:2], 3'b000});

			// lwsp
			16'b010_?_?????_?????_10: type_i(OpCode_Load, in[7+:5], 3'b010, 5'b00010, {4'b0000, in[2+:2], in[12], in[4+:3], 2'b00});

			16'b011_?_?????_?????_10: if (rv64)
				// ldsp
				type_i(OpCode_Load, in[7+:5], 3'b011, 5'b00010, {3'b000, in[2+:3], in[12], in[5+:2], 3'b000});
			else
				// flwsp
				type_i(OpCode_LoadFp, in[7+:5], 3'b010, 5'b00010, {4'b0000, in[2+:2], in[12], in[4+:3], 2'b00});
			...
			// jr, jalr
			16'b100_?_?????_00000_10: type_i(OpCode_Jalr, {4'b0000, in[12]}, 3'b000, in[7+:5], 12'b000000000000);

			// mv, add
			16'b100_?_?????_?????_10: type_r(OpCode_Op, in[7+:5], 3'b000, {5{in[12]}} & in[7+:5], in[2+:5], 7'b0000000);

			// fsdsp
			16'b101_??????_?????_10: type_s(OpCode_StoreFp, 3'b011, 5'b00010, in[2+:5], {3'b000, in[7+:3], in[10+:3], 3'b000});

			// swsp
			16'b110_??????_?????_10: type_s(OpCode_Store, 3'b010, 5'b00010, in[2+:5], {4'b0000, in[7+:2], in[9+:4], 2'b00});

			16'b111_??????_?????_10: if (rv64)
				// sdsp
				type_s(OpCode_Store, 3'b011, 5'b00010, in[2+:5], {3'b000, in[7+:3], in[10+:3], 3'b000});
			else
				// fswsp
				type_s(OpCode_StoreFp, 3'b010, 5'b00010, in[2+:5], {4'b0000, in[7+:2], in[9+:4], 2'b00});
...
module test_rv_decompressor_priority32;
	...
	initial begin
		...
		// Zcb
		test_err(32'b100_0000000000000);
