module rv_decompressor_priority (
	...
	always_comb begin
		...
		priority casez (in[0+:16])
			...
			// flw
			16'b011_???_???_??_???_00: type_i(OpCode_LoadFp, {2'b01, in[2+:3]}, 3'b010, {2'b01, in[7+:3]}, 12'({in[5], in[10+:3], in[6], 2'b00}));
			...
			// fsw
			16'b111_???_???_??_???_00: type_s(OpCode_StoreFp, 3'b010, {2'b01, in[7+:3]}, {2'b01, in[2+:3]}, 12'({in[5], in[10+:3], in[6], 2'b00}));
			...
			// flwsp
			16'b011_?_?????_?????_10: type_i(OpCode_LoadFp, in[7+:5], 3'b010, 5'b00010, 12'({in[2+:2], in[12], in[4+:3], 2'b00}));
			...
			// fswsp
			16'b111_??????_?????_10: type_s(OpCode_StoreFp, 3'b010, 5'b00010, in[2+:5], 12'({in[7+:2], in[9+:4], 2'b00}));
