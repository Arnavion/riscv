...
module booth_multiplier_multi_cycle #(
	...
	multiplier_round multiplier_round_module (
		m, m_is_signed,
		r, r_is_signed,
		i == '0, p,
		mulw, mul, mulh,
...
module multiplier_round #(
	...
	adder #(.width(width)) p_plus_inner_module (
		p_sub, p2, m_maybe_neg,
		p_plus_inner_sum, p_plus_inner_cout
	);
	assign p_plus = {
...
module adder #(
	...
	assign {cout, sum} = {1'b0, a} + {1'b0, b} + (width + 1)'(cin);
endmodule
...
module test_booth_multiplier_multi_cycle;
	...
	`define test_case(m_, m_is_signed_, r_, r_is_signed_, expected_mulw, expected_mul, expected_mulh) begin \
		m = m_; \
