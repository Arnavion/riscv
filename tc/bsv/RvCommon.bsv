...
function OpCode opcode_load(Bit#(1) fp);
	return unpack({ 4'b0000, fp });
endfunction

function OpCode opcode_opimm(Bit#(1) w);
	return unpack({ 3'b001, w, 1'b0 });
endfunction

function OpCode opcode_store(Bit#(1) fp);
	return unpack({ 4'b0100, fp });
endfunction
...
typedef union tagged {
	...
	struct { BranchOp op; rs1_src rs1; rs2_src rs2; Int#(32) imm; } Branch;
...
provisos (
	...
	function Bit#(inst_len) pack(Instruction#(rs1_src, rs2_src, csr_src) inst);
		...
		case (inst) matches
			...
			tagged Branch { op: .op, rs1: .rs1, rs2: .rs2, imm: .imm }: begin
