...
module mkRvDecoder(RvDecoder);
	...
	RWire#(RvDecoderResponse) result <- mkRWire;

	rule run;
		result.wset(RvDecoderResponse { inst: decode(in) });
	endrule

	interface Put request;
		method Action put(RvDecoderRequest request);
			in <= request.in;
		endmethod
	endinterface
	...
	if (unpack(~& in[1:0]))
		...
		case (opcode) matches
			...
			OpCode_Branch: case (parse_branch_op(funct3)) matches
				...
				tagged Valid .op: return tagged Valid tagged Branch { op: op, rs1: rs1, rs2: tagged Left rs2, imm: b_imm };
			endcase
