...
module mkRvDecompressorPriority#(parameter Bool rv64)(RvDecompressor);
	...
	rule lbu(in[15:0] matches 16'b100_000_???_??_???_00);
		out <= type_i_(
			OpCode_Load,
			{ 2'b01, in[4:2] },
			3'b100,
			{ 2'b01, in[9:7] },
			zeroExtend({ in[5], in[6] })
		);
	endrule

	rule lhu_lh(in[15:0] matches 16'b100_001_???_?_?_???_00);
		out <= type_i_(
			OpCode_Load,
			{ 2'b01, in[4:2] },
			{ ~in[6], 2'b01 },
			{ 2'b01, in[9:7] },
			zeroExtend({ in[5], 1'b0 })
		);
	endrule

	rule sb(in[15:0] matches 16'b100_010_???_??_???_00);
		out <= type_s_(
			OpCode_Store,
			3'b000,
			{ 2'b01, in[9:7] },
	...
	rule sh(in[15:0] matches 16'b100_011_???_0_?_???_00);
		out <= type_s_(
			OpCode_Store,
			3'b001,
			{ 2'b01, in[9:7] },
	...
	rule mul(in[15:0] matches 16'b100_111_???_10_???_01);
		out <= type_r_(
			OpCode_Op,
			{ 2'b01, in[9:7] },
			3'b000,
