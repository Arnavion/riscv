...
module mkRvDecompressorPriority(RvDecompressor);
	...
	Wire#(RvDecompressorResponse) result <- mkDWire(RvDecompressorResponse { inst: tagged Invalid });
	Wire#(Bit#(30)) out <- mkWire;

	rule prepare_result;
		result <= RvDecompressorResponse {
			inst: tagged Valid (
				unpack(& in[1:0]) ?
	...
	rule addi4spn(in[15:0] matches 16'b000_????????_000_00 &&& in[12:5] != 0);
		out <= type_i_(
			OpCode_OpImm,
			{ 2'b01, in[4:2] },
			3'b000,
	...
	rule fld_lw_flw(in[15:0] matches 16'b0??_???_???_??_???_00 &&& unpack(| in[14:13]));
		out <= type_i_(
			opcode_load(in[13]),
			{ 2'b01, in[4:2] },
			{ 2'b01, ~in[14] },
	...
	rule fsd_sw_fsw(in[15:0] matches 16'b1??_???_???_??_???_00 &&& unpack(| in[14:13]));
		out <= type_s_(
			opcode_store(in[13]),
			{ 2'b01, ~in[14] },
			{ 2'b01, in[9:7] },
	...
	rule addi_li(in[15:0] matches 16'b0?0_?_?????_?????_01);
		out <= type_i_(
			OpCode_OpImm,
			in[11:7],
			3'b000,
	...
	rule jal_j(in[15:0] matches 16'b?01_???????????_01);
		out <= type_j_(
			OpCode_Jal,
			{ 4'b0000, ~in[15] },
	...
	rule sub(in[15:0] matches 16'b100_011_???_00_???_01);
		out <= type_r_(
			OpCode_Op,
			{ 2'b01, in[9:7] },
			3'b000,
	...
	rule xor_(in[15:0] matches 16'b100_011_???_01_???_01);
		out <= type_r_(
			OpCode_Op,
			{ 2'b01, in[9:7] },
			3'b100,
	...
	rule or_and(in[15:0] matches 16'b100_011_???_1?_???_01);
		out <= type_r_(
			OpCode_Op,
			{ 2'b01, in[9:7] },
			{ 2'b11, in[5] },
	...
	rule fldsp_lwsp_flwsp(in[15:0] matches 16'b0??_?_?????_?????_10 &&& unpack(| in[14:13]));
		out <= type_i_(
			opcode_load(in[13]),
			in[11:7],
			{ 2'b01, ~in[14] },
	...
	rule fsdsp_swsp_fswsp(in[15:0] matches 16'b1??_??????_?????_10 &&& unpack(| in[14:13]));
		out <= type_s_(
			opcode_store(in[13]),
			{ 2'b01, ~in[14] },
			5'b00010,
