...
module mkRvFpu(RvFpu);
	...
	rule convert_2(args.first matches tagged Convert { rm: .rm, out: .width });
		match UnpackResponse { value: .value } = unpack_arg1.response.first;
		pack_result.request.put(PackRequest { width: width, rm: rm, value: value, was_divide_by_zero: False });
	endrule
	...
	rule multiply_3(args.first matches tagged Multiply { rm: .rm, width: .width });
		...
		pack_result.request.put(PackRequest { width: width, rm: rm, value: product, was_divide_by_zero: False });
	endrule
...
provisos (
	...
	match UnpackedInner { sign: .sign, exponent: .exponent, significand: .significand } = value;
	...
	case ({ & exponent, | significand }) matches
		...
		default: begin
			...
			match { .exponent, .significand_d } = normalize_significand(exponent_, unpack(significand_c));
...
module mkPack(Pack);
	...
	rule run(args.first matches PackRequest { width: .width, rm: .rm, value: .value, was_divide_by_zero: .was_divide_by_zero });
		case (width) matches
			H: begin
				Tuple2#(UnpackedInner#(`H_EXPONENT_LEN, `H_SIGNIFICAND_LEN), Flags) rounded = round(rm, value, was_divide_by_zero);
			...
			S: begin
				Tuple2#(UnpackedInner#(`S_EXPONENT_LEN, `S_SIGNIFICAND_LEN), Flags) rounded = round(rm, value, was_divide_by_zero);
				match { .rounded_value, .flags } = rounded;
				result.put(PackResponse {
					result: {
						rounded_value.upper,
			...
			D: begin
				Tuple2#(UnpackedInner#(`D_EXPONENT_LEN, `D_SIGNIFICAND_LEN), Flags) rounded = round(rm, value, was_divide_by_zero);
				match { .rounded_value, .flags } = rounded;
				result.put(PackResponse {
					result: {
						rounded_value.upper,
...
function Tuple2#(UnpackedInner#(exponent_len, significand_len), Flags) round(
	RoundingMode rm,
	Unpacked value,
	Bool was_divide_by_zero
)
provisos (
	...
	case (value) matches
		tagged Finite { sign: .sign, exponent: .exponent_, significand: .significand_ }: begin
			...
			begin
				match { .normalized_exponent, .normalized_significand } = normalize_finite(exponent_len, exponent, significand);
				...
				if (rounded_significand_ != normalized_significand)
					inexact = True;
...
function Tuple2#(Int#(`I_EXPONENT_LEN), UInt#(`I_SIGNIFICAND_LEN)) normalize_significand(
	Int#(`I_EXPONENT_LEN) exponent,
	UInt#(`I_SIGNIFICAND_LEN) significand
);
	...
	if (significand_ne0) begin
		...
		case (compare(shift_by, 0)) matches
			...
			// 2e19 -> 1e20 until 1eX
			GT: begin
				significand = shift_right_accumulate(significand, shift_by);
				exponent = exponent + shift_by;
			end
		endcase
...
function UInt#(t) shift_right_accumulate(UInt#(t) value, s shamt)
provisos (
	Add#(1, a__, t),
	...
	case (tuple2(arg1, arg2)) matches
		{
			...
			let product_significand = truncate(shift_right_accumulate(arg1_significand * arg2_significand, valueOf(`I_FRACTION_LEN)));
			match { .normalized_exponent, .normalized_significand } = normalize_significand(product_exponent, product_significand);
		...
		{ tagged Finite { sign: .sign1, significand: .significand }, tagged Infinity { sign: .sign2 } }:
			if (significand == 0)
				return tagged NaN { sign: ?, quiet: False };
			else
				return tagged Infinity { sign: unpack(pack(sign1) ^ pack(sign2)) };

		{ tagged Finite ._, tagged NaN { quiet: .quiet } }:
			return tagged NaN { sign: ?, quiet: quiet };
		...
		{ tagged Infinity ._, tagged NaN { quiet: .quiet } }:
			return tagged NaN { sign: ?, quiet: quiet };

		{ tagged NaN { quiet: .quiet }, tagged Finite ._ }:
			return tagged NaN { sign: ?, quiet: quiet };

		{ tagged NaN { quiet: .quiet }, tagged Infinity ._ }:
			return tagged NaN { sign: ?, quiet: quiet };

		{ tagged NaN { quiet: .quiet1 }, tagged NaN { quiet: .quiet2 } }:
			return tagged NaN { sign: ?, quiet: quiet1 && quiet2 };
	endcase
endfunction
...
module mkSignInject(SignInject);
	...
	rule run(args.first matches SignInjectRequest { op: .op, width: .width, arg1: .arg1, arg2: .arg2 });
		let arg1_sign_injected = case (width) matches
			H: begin
				UnpackedInner#(`H_EXPONENT_LEN, `H_SIGNIFICAND_LEN) arg2_unpacked = unpack_inner1(arg2);
				let arg2_sign =
					(arg2_unpacked.upper == '1) ?
						arg2_unpacked.sign :
						'0; // Canonical NaN's sign

				UnpackedInner#(`H_EXPONENT_LEN, `H_SIGNIFICAND_LEN) arg1_unpacked = unpack_inner1(arg1);
			...
			S: begin
				UnpackedInner#(`S_EXPONENT_LEN, `S_SIGNIFICAND_LEN) arg2_unpacked = unpack_inner1(arg2);
				let arg2_sign =
					(arg2_unpacked.upper == '1) ?
						arg2_unpacked.sign :
						'0; // Canonical NaN's sign

				UnpackedInner#(`S_EXPONENT_LEN, `S_SIGNIFICAND_LEN) arg1_unpacked = unpack_inner1(arg1);
				if (arg1_unpacked.upper != '1) begin
					// Canonical NaN
					arg1_unpacked.upper = '1;
					arg1_unpacked.sign = '0;
			...
			D: begin
				UnpackedInner#(`D_EXPONENT_LEN, `D_SIGNIFICAND_LEN) arg2_unpacked = unpack_inner1(arg2);
				let arg2_sign = arg2_unpacked.sign;

				UnpackedInner#(`D_EXPONENT_LEN, `D_SIGNIFICAND_LEN) arg1_unpacked = unpack_inner1(arg1);
