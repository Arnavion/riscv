...
mod awint {
	...
	type Awi131 = awint::inlawi_ty!(131);
	...
	#[derive(Clone, Copy, Debug)]
	#[repr(transparent)]
	pub(crate) struct I131(Awi131);

	impl I131 {
		pub(super) fn field_to_1(i: I65) -> Self {
			Self(awint::inlawi!(0u65, &i.0, 0u1 ; ..131).unwrap())
		}
		...
		pub(super) fn add_upper(
			...
			let mut lhs = awint::inlawi!(&self.0[66..131]).unwrap();
			lhs.neg_add_(neg, &rhs.0).unwrap();
			Self(awint::inlawi!(lhs, &self.0[..66] ; ..131).unwrap())
		}
	}

	impl std::ops::Shr<usize> for I131 {
		type Output = Self;
...
use awint::I65;
pub(crate) use awint::I131;

#[derive(Debug)]
pub(crate) enum State {
	Pending { i: u8, p: I131 },
	Mulw { i: u8, p: I131, mulw: i32 },
	Mul { mul: i64, mulh: i64 },
}

impl State {
	pub(crate) fn initial(r_is_signed: bool, r: i64) -> (u8, I131) {
...
pub(crate) fn round(
	...
	let a: I65 = if m_is_signed { m.into() } else { m.cast_unsigned().into() };
