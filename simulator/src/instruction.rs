...
pub(crate) enum Instruction {
	...
	Csrrw { rd: XReg, rs1: XReg, csr: Csr },
	Csrrwi { rd: XReg, imm: i64, csr: Csr },
	Csrrs { rd: XReg, rs1: XReg, csr: Csr },
	Csrrsi { rd: XReg, imm: i64, csr: Csr },
	Csrrc { rd: XReg, rs1: XReg, csr: Csr },
	Csrrci { rd: XReg, imm: i64, csr: Csr },
	Ebreak,
	Fence,
	Jal { rd: XReg, imm: i64 },
		...
		Ok(match inst & 0b11 {
			...
			0b11 => {
				let inst = match (inst >> 2) & 0x1f {
					...
					0b00100 => {
						let FormattedInstructionI { rd, rs1, funct3, imm } = inst.into();
						let op = OpImmOp::from((funct3, imm));
...
impl From<(u8, i64)> for OpImmOp {
	fn from((funct3, imm): (u8, i64)) -> Self {
		match funct3 {
			0b000 => Self::Addi,
			0b001 => match (imm >> 6) & 0x3f {
				...
				0b001010 => Self::Bseti,
				0b010010 => Self::Bclri,
				funct7 @ 0b011000 => match imm & 0x3f {
					0b000000 => Self::Clz,
					0b000001 => Self::Ctz,
					0b000010 => Self::Cpop,
					0b000100 => Self::SextB,
					0b000101 => Self::SextH,
					funct5 => panic!("invalid (funct3, funct7, funct5) for OP-IMM: (0b{funct3:03b}, 0b{funct7:06b}, 0b{funct5:06b})"),
				},
				0b011010 => Self::Binvi,
				funct7 => panic!("invalid (funct3, funct7) for OP-IMM: (0b{funct3:03b}, 0b{funct7:06b})"),
			},
			0b010 => Self::Slti,
			0b011 => Self::Sltiu,
			0b100 => Self::Xori,
			0b101 => match (imm >> 6) & 0x3f {
				0b000000 => Self::Srli,
				funct7 @ 0b001010 => match imm & 0x3f {
					0b000111 => Self::OrcB,
					funct5 => panic!("invalid (funct3, funct7, funct5) for OP-IMM: (0b{funct3:03b}, 0b{funct7:06b}, 0b{funct5:06b})"),
				},
				0b010000 => Self::Srai,
				0b010010 => Self::Bexti,
				0b011000 => Self::Rori,
				funct7 @ 0b011010 => match imm & 0x3f {
					0b111000 => Self::Rev8,
					funct5 => panic!("invalid (funct3, funct7, funct5) for OP-IMM: (0b{funct3:03b}, 0b{funct7:06b}, 0b{funct5:06b})"),
				},
				funct7 => panic!("invalid (funct3, funct7) for OP-IMM: (0b{funct3:03b}, 0b{funct7:06b})"),
			...
			0b110 => Self::Ori,
			0b111 => Self::Andi,
			funct3 => panic!("invalid funct3 for OP-IMM: 0b{funct3:03b}"),
		}
	}
}
...
impl From<(u8, i64)> for OpImm32Op {
	fn from((funct3, imm): (u8, i64)) -> Self {
		match funct3 {
			0b000 => Self::Addiw,
			0b001 => match (imm >> 5) & 0x7f {
				...
				funct7 @ 0b0110000 => match imm & 0x1f {
					0b00000 => Self::Clzw,
					0b00001 => Self::Ctzw,
					0b00010 => Self::Cpopw,
					funct5 => panic!("invalid (funct3, funct7, funct5) for OP-IMM-32: (0b{funct3:03b}, 0b{funct7:07b}, 0b{funct5:05b})"),
				},
				funct7 => panic!("invalid (funct3, funct7) for OP-IMM-32: (0b{funct3:03b}, 0b{funct7:07b})"),
			},
			...
			funct3 => panic!("invalid funct3 for OP-IMM-32: 0b{funct3:03b}"),
		}
	}
}
...
impl From<u8> for BranchOp {
	fn from(funct3: u8) -> Self {
		match funct3 {
			0b000 => Self::Equal,
			0b001 => Self::NotEqual,
			0b100 => Self::LessThan,
			...
			funct3 => panic!("invalid funct3 for BranchOp: 0b{funct3:03b}"),
		}
	}
}
