...
pub(crate) enum Ucode {
	...
	Mul { op: MulOp, rd: (XReg, Tag, Option<i64>), rs1: RegisterValue, rs2: RegisterValue, state: Option<(u8, I131)> },
	...
	pub(crate) fn new(
		...
		fn assign_x_reg(
			...
			match rs {
				RegisterValue::Value(value) => {
					let (rd, tag, _) = try_rename_xreg(rd, x_regs, tag)?;
		...
		match inst {
			Instruction::Abs { rd, rs } => {
				...
				let rd = try_rename_xreg(rd, x_regs, tag_czero)?;
			...
			Instruction::Auipc { rd, imm } => {
				...
				let rd = try_rename_xreg(rd, x_regs, tag_add)?;
			...
			Instruction::Load { op, rd, base: MemoryBase::XReg(base), offset: MemoryOffset::Imm(0) } => {
				...
				let rd = try_rename_xreg(rd, x_regs, tag_load)?;
			...
			Instruction::Op { op, rd, rs1, rs2 } => {
				...
				let rs2 = x_regs.load(rs2);
				let (tag_op, tags) = tags.allocate();
				let rd = try_rename_xreg(rd, x_regs, tag_op)?;
				match op {
			...
			Instruction::Op32 { op: Op32Op::AddUw, rd, rs1, rs2: XReg::X0 } => {
				let rs1 = x_regs.load(rs1);
				let (tag_li, _) = tags.allocate();
				let rd = try_rename_xreg(rd, x_regs, tag_li)?;
